...
public class Java125_RegEx {
	...
	public static void main(String[] args) {
		...
		System.out.println(sn.matches(".*[\\d].*")); //문자열 \ 사용
		...
		//st변수에 저장된 값이 a나 b로 시작되고 임의문자가 5개이면 true 아니면 false를 리턴한다. 
		System.out.println(st.matches("[ab].{5}"));
		
		//st변수에 저장된 값이 a나 b로 시작되고 임의문자가 1개에서 5개이며 cc로 끝나면 
		...
		System.out.println(mc.find(6)); //인덱스 6이후로 d(숫자)가 있는지
		System.out.println(mc.find(3)); //인덱스 3이후로 d(숫자)가 있는지
		...
		input1 = "abced3gs";		
		System.out.print(input1.matches("[a-zA-Z_0-9]{5,10}")
